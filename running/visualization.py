...
def log_hyperparams_and_metrics(config: Config,
    ...
    for team_idx, team_name in enumerate(TEAM_NAMES):
        metrics    [f'final_comparison/{team_name}s-winrate-against_training_opponents'] = selves_wons[team_idx].mean()
        if others_wons is not None:
            metrics[f'final_comparison/{team_name}s-winrate-against_external_opponents'] = others_wons[team_idx].mean()

        for op in DESCRIPTIVE_STATS:
            metrics[f'final_comparison/{team_name}s-reward_{op}-against_training_opponents'] = getattr(selves_rewards, op)()
            if others_rewards is not None:
                metrics[f'final_comparison/{team_name}s-winrate-against_external_opponents'] = getattr(others_rewards, op)()
