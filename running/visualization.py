...
def log_hyperparams_and_metrics(config: Config,
    ...
    for team_idx, team_name in enumerate(TEAM_NAMES):
        metrics    [f'final_comparison/{team_name}s-winrate-against_training_opponent'] = selves_wons[team_idx].mean()
        ...
        for op in DESCRIPTIVE_STATS:
            metrics[f'final_comparison/{team_name}s-reward_{op}-against_training_opponent'] = getattr(selves_rewards, op)()
